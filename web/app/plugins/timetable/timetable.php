...
<p>Selected event hour doesn't exist.<br>Please select different event.</p>", "timetable"), array("h2" => array(), "p" => array(), "br" => array()));
		...
		if(!$user_id && $booking_form_config['allow_guest_booking']=='yes')
		{
		    ...
		    if($booking_form_config['default_booking_view']=='guest')
		        ...
		        $result['msg'] .= sprintf(wp_kses(__('<p class="tt_guest_option ' . ($guestOptionHidden ? 'tt_hide' : '') . '">Don\'t have an account? <a href="%s">Continue as guest</a></p>', 'timetable'), array("a" => array("href" => array()), "p" => array())), '#');
		}
		...
		if(!$user_id && $booking_form_config['allow_user_booking']=='yes')
		{
		    ...
		    if($booking_form_config['default_booking_view']=='user')
		        ...
		        $result['msg'] .= sprintf(wp_kses(__('<p class="tt_login_option ' . ($loginOptionHidden ? 'tt_hide' : '') . '">Got an account? <a href="%s">Login</a></p>', 'timetable'), array("a" => array("href" => array()), "p" => array())), wp_login_url($redirect_url));
