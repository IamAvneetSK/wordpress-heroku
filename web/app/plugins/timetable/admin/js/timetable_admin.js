...
//						$("#event_hours_table #add_event_hours").after("<input type='hidden' id='event_hours_id' name='event_hours_id' value='" + id + "' />");
	...
	//widget
	$("[id*='timetable_upcoming_events'] [name$='[time_format]']").change(function(){
		if($(this).val()!="custom")
		{
	...
	$("[id*='timetable_upcoming_events'] [name$='[time_format_custom]']").on('focus', function(){
		$(this).prev().children().prop("checked", true);
	});
	$("[id*='timetable_upcoming_events'] [name$='[time_format_custom]']").on('change', function(){
		var format = $(this).val();
		var self = $(this);
		$.ajax({
				url: ajaxurl,
	...
	$("[id*='timetable_upcoming_events'] [name$='[display_settings]']").change(function(){
		if($(this).val()=="all")
	...
	$(document).on("widget-added", function(event, widget) {
       var widget_index = widget.attr("id").split("-")[2];
		$("[name='widget-timetable_upcoming_events[" + widget_index + "][time_format]']").change(function(){
			if($(this).val()!="custom")
			{
				$(this).parent().siblings("input:last").val($(this).val());
				$(this).parent().siblings(".example").html($(this).next().html());
		...
		$("[name='widget-timetable_upcoming_events[" + widget_index + "][time_format_custom]']").on('focus', function(){
			$(this).prev().children().prop("checked", true);
		});
		$("[name='widget-timetable_upcoming_events[" + widget_index + "][time_format_custom]']").on('change', function(){
			var format = $(this).val();
			var self = $(this);
			$.ajax({
					url: ajaxurl,
		...
		$("#widget-timetable_upcoming_events-" + widget_index + "-display_settings").change(function(){
			if($(this).val()=="all")
				$(this).parent().next().css("display", "block");
			else
				$(this).parent().next().css("display", "none");
